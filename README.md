...
2. **修改代码**
   - 删掉 `CheckIntergrity.kt` 里的签名检测代码
   - 删掉加载广告的代码
   - 删掉 `anti_debug.rs` 里的反调试相关代码
   - 删掉 `anti_debug.rs` 以及 `ad_track.rs` 里的反 hook 广告代码
3. **构建**
   构建前记得把 `app\src\main\java\com\coc\zkqcode\jar` 删掉，构建完就能跑了。
...
本项目已在 Gitee 平台开源，所有源代码公开透明，接受社区审查。本软件仅通过图色识别与模拟点击的方式，复现新手玩家的基本操作流程，不涉及任何内存读写、协议篡改或数据注入等违规/作弊行为。所有权限（即Root权限）均由用户手动授予，合法合规。本质原理和用户对游戏进行录屏，或者开鼠标宏操作一样，完全绿色安全。本软件无侵入性功能，不会对游戏内的对战环境产生任何影响，不影响游戏公平性。
