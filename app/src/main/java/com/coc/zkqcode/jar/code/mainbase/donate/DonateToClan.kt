...
suspend fun donateToClan(): Boolean {
    ...
    val lowerThreshold =
        getConfigRuntime(Schema.MAIN_BASE_SETTINGS.DONATION_FARMING_START_THRESHOLD.key).toIntOrNull() ?: logAndRestart("${Schema.MAIN_BASE_SETTINGS.DONATION_FARMING_START_THRESHOLD.displayName} 必须是数字，请检查配置")
    val higherThreshold =
        getConfigRuntime(Schema.MAIN_BASE_SETTINGS.DONATION_FARMING_STOP_THRESHOLD.key).toIntOrNull() ?: logAndRestart("${Schema.MAIN_BASE_SETTINGS.DONATION_FARMING_STOP_THRESHOLD.displayName} 必须是数字，请检查配置")
    ...
    while (System.currentTimeMillis() - startTime < intervalMillis) {
        ...
        ShowMessage("捐兵检测中，剩余${remainingSeconds}秒结束")
...
private suspend fun donateActions() {
    ...
    val donationSchemas = listOf(
        MyColors.DonateSuperTroops,
        MyColors.DonateNormalTroops,
        MyColors.DonateSpells
    )
    repeat(5) {
