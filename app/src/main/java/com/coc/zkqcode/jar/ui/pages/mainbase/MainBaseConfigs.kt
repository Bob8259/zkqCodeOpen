...
fun LazyListScope.MainBaseConfig(
    ...
    if (isExpanded) {
        ...
        item { SettingCheckBox(key = "${MAIN_BASE_SETTINGS.AUTO_ATTACK.key}_c$index") }
        item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.GOLD_REQUIREMENT.key}_c$index") }
        item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.ELIXIR_REQUIREMENT.key}_c$index") }
        item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.DARK_ELIXIR_REQUIREMENT.key}_c$index") }
        item {
            FlowRow {
                SettingCheckBox(
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.PLAY_LADDER.key}_c$index"]?.value == "1") {
            item { SettingCheckBox(key = "${MAIN_BASE_SETTINGS.CHANGE_BASE.key}_c$index") }
        }
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.AI_DEPLOY_TROOPS.key}_c$index"]?.value == "0") {
            item {
                SettingDropdown(
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.AI_DEPLOY_TROOPS.key}_c$index"]?.value == "1") {
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.LIGHTING_ON_AIR_SWEEPER.key}_c$index") }
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.LIGHTING_ON_AIR_DEFENCE.key}_c$index") }
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.LIGHTING_ON_WIZARD_TOWER.key}_c$index") }
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.LIGHTING_ON_MORTAR.key}_c$index") }
        }
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.DONATION_SETTING.key}_c$index"]?.value == "1") {
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.DONATION_TIMES.key}_c$index") }
        }

        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.RESEARCH_SETTING.key}_c$index"]?.value == "1") {
            item { ResearchConfigs(index) }
        }
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.WALL_UPGRADE_SETTINGS.key}_c$index"]?.value == "1") {
            item {
                FlowRow {
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.BUILD_SETTING.key}_c$index"]?.value == "1") {
            item { UpgradeConfigs(index, onNavigatePriority) }
        }

        item { SettingCheckBox(key = "${MAIN_BASE_SETTINGS.UPGRADE_PETS.key}_c$index") }
        
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.UPGRADE_PETS.key}_c$index"]?.value == "1") {
            item { PetConfigs(index) }
        }
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.AUTO_JOIN_CLAN.key}_c$index"]?.value == "1") {
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.CLAN_TAG.key}_c$index") }
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.CLAN_JOIN_MESSAGE.key}_c$index") }
        }
        ...
        if (GlobalVars.configStates["${MAIN_BASE_SETTINGS.CREATE_CONSECUTIVE_CLANS.key}_c$index"]?.value == "1") {
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.CLAN_NAME.key}_c$index") }
            item { SettingInputRow(key = "${MAIN_BASE_SETTINGS.CONSECUTIVE_COUNT.key}_c$index") }
        }
    }
}
